...
using Sels.Core.Data.MySQL.Models.Data;
...
namespace Sels.Core.Data.MySQL.Templates.Service.MariaDb
{
    /// <summary>
    /// Template for creating a new <see cref="IDataService{TEntity, TId}"/> using MariaDb as the backing database.
    ...
    public abstract class BaseMariaDbDataService<TEntity, TId> : IDataService<TEntity, TId>
    {
        // Fields
        ...
        /// <inheritdoc cref="BaseMariaDbDataService{TEntity, TId}"/>
        ...
        public BaseMariaDbDataService(string connectionString, IEnumerable<ILogger>? loggers = null)
        {
            ...
            IdGetter = new Lazy<Func<TEntity, TId>>(() =>
            {
                ...
                var memberExpression = LinqExpression.Property(parameterExpression, IdProperty.Value);
        ...
        /// <inheritdoc/>
        public async Task<IDataServiceConnection> OpenNewConnectionAsync(bool startTransaction = true, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                _loggers.Debug($"Opening new connection from <{_name}>");
                var connection = new MySqlDataServiceConnection(_connectionString, startTransaction);
        ...
        #region Create
        /// <inheritdoc/>
        public virtual async Task<TEntity> CreateAsync(IDataServiceConnection connection, TEntity entity, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                ...
                // Create query
                var query = MySql.Insert<TEntity>().Into()
                                    .ParametersFrom(excludedProperties: excludedProperties)
                                    .Return(x => x.All())
                                    .Build(QueryBuilderOptions.Format);
        ...
        /// <inheritdoc/>
        public virtual async Task<TEntity[]> CreateAsync(IDataServiceConnection connection, IEnumerable<TEntity> entities, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        #region Read
        /// <inheritdoc/>
        public virtual async Task<bool> ExistsAsync(IDataServiceConnection connection, TId id, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        /// <inheritdoc/>
        public virtual async Task<TEntity[]> GetAllAsync(IDataServiceConnection connection, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        /// <inheritdoc/>
        public virtual async Task<TEntity> GetAsync(IDataServiceConnection connection, TId id, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        #region Update
        /// <inheritdoc/>
        public virtual async Task<TEntity> UpdateAsync(IDataServiceConnection connection, TEntity entity, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        /// <inheritdoc/>
        public virtual async Task<TEntity[]> UpdateAsync(IDataServiceConnection connection, IEnumerable<TEntity> entities, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
        ...
        #region Delete
        /// <inheritdoc/>
        public virtual async Task<TEntity> DeleteAsync(IDataServiceConnection connection, TId id, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
                ...
                // Create parameters
                var parameters = new DynamicParameters();
                parameters.Add(IdProperty.Value.Name, id);
        ...
        /// <inheritdoc/>
        public virtual async Task<TEntity[]> DeleteAsync(IDataServiceConnection connection, IEnumerable<TId> ids, CancellationToken token = default)
        {
            using (_loggers.TraceMethod(this))
            {
                connection.ValidateArgument(nameof(connection));
                ...
                // Create parameters
                var parameters = new DynamicParameters();
                ids.Execute((i, x) => parameters.Add(idName + i, x));
