    ...
    public static class LinuxHelper
    {
        public static class Command
        {
            ...
            public static string BuildLinuxArguments(ICommand command, IEnumerable<(string Argument, int Order)> additionalArguments = null, IEnumerable <ILogger> loggers = null)
            {
                ...
                using var logger = loggers.CreateTimedLogger(LogLevel.Debug, $"Building linux arguments for command {commandName}", x => $"Built linux arguments for command {command.GetType().Name} in {x.PrintTotalMs()}");
                ...
                var propertyArguments = command.GetProperties().ToDictionary(x => x, x => x.GetCustomAttributes().FirstOrDefault(a => a.IsAssignableTo<LinuxArgument>()).AsOrDefault<LinuxArgument>()).Where(x => x.Value.HasValue()).Select(x => (Argument: x.Value.GetArgument(x.Key.Name, x.Key.GetValue(command)), Order: x.Value.Order)).ToList();
        ...
        public static class Program
        {
            ...
            public static bool Run(string program, string arguments, out string output, out string error, out int exitCode, int succesExitCode = LinuxConstants.SuccessExitCode, CancellationToken token = default, IEnumerable<ILogger> loggers = null)
            {
            ...
            /// <param name="succesExitCode">Exit code indicating succesful execution</param>
            /// <returns>Boolean indicating if the command was executed successfully</returns>
            public static bool Run(string command, out string output, out string error, out int exitCode, int succesExitCode = LinuxConstants.SuccessExitCode, CancellationToken token = default, IEnumerable<ILogger> loggers = null)
            {
                command.ValidateArgument(nameof(command));

                return Run(LinuxConstants.Commands.Shell, FormatStringCommand(command), out output, out error, out exitCode, succesExitCode, token, loggers);
            }
            ...
            public static class Bash
            {
                ...
                public static bool Run(string command, out string output, out string error, out int exitCode, int succesExitCode = LinuxConstants.SuccessExitCode, CancellationToken token = default, IEnumerable<ILogger> loggers = null)
                {
                    ...
                    return Program.Run(LinuxConstants.Commands.Bash, FormatStringCommand(command), out output, out error, out exitCode, succesExitCode, token, loggers);
                }
            }
            #endregion
        }
